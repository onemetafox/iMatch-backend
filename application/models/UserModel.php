...
class UserModel  extends CI_Model
{
  ...
  public function get_personalmatch() //the matches that i have done competing my own medias.
  {
    ...
    $result = $this->db->query("select * from tb_match WHERE rival_id='$user_id' and opponent_id='$user_id'");
    if ($this->db->affected_rows() > 0) {
      $result_array = $result->result_array();
      $data = array();
      foreach ($result_array as $result) {
        ...
        if (!empty($outputdata->profile_pic)) {
          $strings = $outputdata->profile_pic;
          if (preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $strings)) {
            // one or more of the 'special characters' found in $string
            // echo "found";
            $picture = $outputdata->profile_pic;
          } else {
            // echo "not found";
            $picture = base_url() . 'uploads/profile_image/' . $outputdata->profile_pic;
          }
        } else {
          // echo "not found";
  ...
  public function get_personalmatch_link() //the matches that i have done competing my own medias.
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE rival_id='$user_id' and opponent_id='$user_id' and tb_matchupload.filetype='link' GROUP by tb_match.matchid");
    $data = array();
    if ($this->db->affected_rows() > 0) {
      $result_array = $result->result_array();
      $data = array();
  ...
  public function get_personalmatch_text() //the matches that i have done competing my own medias.
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE rival_id='$user_id' and opponent_id='$user_id' and tb_matchupload.filetype='text' GROUP by tb_match.matchid");
    $data = array();
    if ($this->db->affected_rows() > 0) {
      $result_array = $result->result_array();
      $data = array();
  ...
  public function get_personalmatch_audio() //the matches that i have done competing my own medias.
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
  ...
  public function get_personalmatch_image() //the matches that i have done competing my own medias.
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    ...
    if ($this->db->affected_rows() > 0) {
      ...
      foreach ($result_array as $result) {
        ...
        $ex_result = explode(".", $fileextention);
        $extension = $ex_result[1];
        if (($extension == 'png') || ($extension == 'jpg') || ($extension == 'jpeg') || ($extension == 'gif') || ($extension == 'gif')) {
          $matchid = $result['matchid'];
          $uid = $result['rival_id'];
          $query = $this->db->query("select * from tb_user WHERE id='$uid'");
          $output = $query->row();
  ...
  public function get_personalmatch_video() //the matches that i have done competing my own medias.
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    ...
    if ($this->db->affected_rows() > 0) {
      ...
      foreach ($result_array as $result) {
        ...
        $ex_result = explode(".", $fileextention);
        $extension = $ex_result[1];
        if (($extension == 'mp4') || ($extension == 'mov') || ($extension == 'wmv') || ($extension == 'flv') || ($extension == 'aiv') || ($extension == 'mkv')) {
          $matchid = $result['matchid'];
          $uid = $result['rival_id'];
          $query = $this->db->query("select * from tb_user WHERE id='$uid'");
          $output = $query->row();
  ...
  public function get_openmatch() //whom have send matches to me 
  {
    ...
    if ($this->db->affected_rows() > 0) {
      ...
      foreach ($result_array as $result) {
        ...
        $matchsenderfilequery = $this->db->query("select * from tb_matchupload WHERE user_uploaded='$rivalid' and matchid=$matchid"); //rival
        if ($matchsenderfilequery->num_rows() > 0) {
          $senderfileoutput = $matchsenderfilequery->row();
        ...
        $matchoppofilequery = $this->db->query("select * from tb_matchupload WHERE user_uploaded='$oppoid' and matchid=$matchid"); //rival
        if ($matchoppofilequery->num_rows() > 0) {
          $oppofileoutput = $matchoppofilequery->row();
  ...
  public function get_openmatch_link() //whom have send matches to me 
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $userid = $this->input->post('userid');
  ...
  public function get_openmatch_text() //whom have send matches to me 
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $userid = $this->input->post('userid');
    $data = array();
    // echo "select * from tb_match WHERE (rival_id='$userid' or opponent_id='$userid') and invitation_status='accept' and '$current_date' between replied_at and match_end";die();
    $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE opponent_id='$userid' and invitation_status='accept' and match_end <'$current_date' and tb_matchupload.filetype='text' GROUP by tb_match.matchid");
    if ($this->db->affected_rows() > 0) {
      $result_array = $result->result_array();
  ...
  public function get_openmatch_audio() //whom have send matches to me 
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $userid = $this->input->post('userid');
    // echo "select * from tb_match WHERE (rival_id='$userid' or opponent_id='$userid') and invitation_status='accept' and '$current_date' between replied_at and match_end";die();
    $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE opponent_id='$userid' and invitation_status='accept' and match_end <'$current_date' and tb_matchupload.filetype='file' GROUP by tb_match.matchid");
    if ($this->db->affected_rows() > 0) {
      $result_array = $result->result_array();
      $data = array();
      foreach ($result_array as $result) {
  ...
  public function get_openmatch_image() //whom have send matches to me 
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $userid = $this->input->post('userid');
  ...
  public function get_openmatch_video() //whom have send matches to me 
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $userid = $this->input->post('userid');
  ...
  public function get_ongoingmatch()
  {
    ...
    //select * from tb_match join tb_matchupload on tb_match.matchid=tb_match.matchid WHERE (rival_id='$user_id' or opponent_id='$user_id') and invitation_status='accept' and match_end>='$current_date' and tb_matchupload.filetype='link' GROUP by tb_match.matchid
    $result_array = $result->result_array();
    $data = array();
    foreach ($result_array as $result) {
  ...
  public function get_closedmatch_audio() //the matches that i have selected medias from other person's personal match and compete
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
      ...
      if (($extension == 'mp3') || ($extension == 'amr') || ($extension == 'wav') || ($extension == 'wma') || ($extension == 'aac') || ($extension == 'ogg') || ($extension == 'aiff') || ($extension == 'aif')) {
        ...
        if ($oppouid  == $user_id) {
          ...
          if ($seen_status == '1') {
            ...
            if ($matchsenderfilequery->num_rows() > 0) {
              $senderfileoutput = $matchsenderfilequery->row();
              $sfileid = $senderfileoutput->mup_id;
              $sfilename = $senderfileoutput->filename;
              $sfiletype = $senderfileoutput->filetype;
              if ($sfiletype == 'file') {
                $sfile = base_url() . 'uploads/Matchuploads/' . $sfilename;
  ...
  public function get_closedmatch_image() //the matches that i have selected medias from other person's personal match and compete
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
  ...
  public function get_closedmatch_video() //the matches that i have selected medias from other person's personal match and compete
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
  ...
  public function get_closedmatch_link() //the matches that i have selected medias from other person's personal match and compete
  {
    ...
    $user_id = $this->input->post('userid');
    // $visitorid = $this->input->post('visitorid'); //only used to get the liked portion if the logged user is a visitor of other profile.
    $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE (rival_id='$user_id' or opponent_id='$user_id') and match_type='closed' and tb_matchupload.filetype='link' GROUP by tb_match.matchid");
  ...
  public function get_closedmatch_text() //the matches that i have selected medias from other person's personal match and compete
  {
    ...
    $user_id = $this->input->post('userid');
    // $visitorid = $this->input->post('visitorid'); //only used to get the liked portion if the logged user is a visitor of other profile.
    $result = $this->db->query(" select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE (rival_id='$user_id' or opponent_id='$user_id') and match_type='closed' and tb_matchupload.filetype='text' GROUP by tb_match.matchid");
  ...
  public function get_ongoingmatch_link()
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    $visitorid = $this->input->post('visitorid'); //only used to get the liked portion if the logged user is a visitor of other profile.
    if (empty($user_id)) {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='link' group by tb_match.matchid");
    } else {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE (tb_match.rival_id='$user_id' or tb_match.opponent_id='$user_id') and tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='link' group by tb_match.matchid");
    }
    $result_array = $result->result_array();
    $data = array();
    foreach ($result_array as $result) {
  ...
  public function get_ongoingmatch_text()
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    $visitorid = $this->input->post('visitorid'); //only used to get the liked portion if the logged user is a visitor of other profile.
    if (empty($user_id)) {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='text' group by tb_match.matchid");
    } else {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE (tb_match.rival_id='$user_id' or tb_match.opponent_id='$user_id') and tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='text' group by tb_match.matchid");
    }
    $result_array = $result->result_array();
    $data = array();
    foreach ($result_array as $result) {
  ...
  public function get_ongoingmatch_audio()
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
    $visitorid = $this->input->post('visitorid'); //only used to get the liked portion if the logged user is a visitor of other profile.
    if (empty($user_id)) {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='file' group by tb_match.matchid");
    } else {
      $result = $this->db->query("select * from tb_match join tb_matchupload on tb_match.matchid=tb_matchupload.matchid WHERE (tb_match.rival_id='$user_id' or tb_match.opponent_id='$user_id') and tb_match.invitation_status='accept' and tb_match.match_end>='$current_date' and tb_matchupload.filetype='file' group by tb_match.matchid");
    }
    $result_array = $result->result_array();
    $data = array();
    foreach ($result_array as $result) {
  ...
  public function get_ongoingmatch_image()
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
  ...
  public function get_ongoingmatch_video()
  {
    date_default_timezone_set('Asia/Kolkata');
    $current_date    = date('Y-m-d H:i:s');
    $user_id = $this->input->post('userid');
